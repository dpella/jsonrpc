...
RUN --mount=type=cache,id=apt-cache-jsonrpc,target=/var/cache/apt \
    --mount=type=cache,id=apt-libs-jsonrpc,target=/var/lib/apt \
...
COPY --chown=${UID}:${GID} jsonrpc.cabal /app/jsonrpc.cabal
COPY --chown=${UID}:${GID} cabal.project /app/cabal.project

WORKDIR /app
# Using the cabal files, we can build the dependencies
RUN --mount=type=cache,id=jsonrpc-build-cache,uid=${UID},gid=${GID},target=/app/dist-newstyle \
    cabal update && \
